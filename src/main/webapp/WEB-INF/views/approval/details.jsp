...
$(function(){
    ...
    // 검색 버튼 클릭 시 리스트 가져오기
    $('#btnDetailsSearch').on('click', function(){
        keyword = $('#detailsSearch').val();
        getList(currentPage, keyword, tabId);
    });

	// 검색란에서 엔터 키를 누르면 리스트 가져오기
	$('#detailsSearch').on('keyup', function(event){
...
function getList(currentPage, keyword, tabId){
	...
	$.ajax({
		...
		success:function(result){
			...
			let str = "";
			if(result.total == 0){
				str += `<td colspan="6" class="center" style="height:100px">결재내역 문서가 없습니다.</td>`;
			}
					...
					<div class="nav-align-top mb-4">
						<ul class="nav nav-tabs m-0" role="tablist">
							<li class="nav-item">
							  <button type="button" class="nav-link nav-link-appr active" role="tab"
							    data-bs-toggle="tab" data-bs-target="#navs-top-history" data-tab="history"
							    aria-controls="navs-top-home"  aria-selected="true">전체</button>
							</li>
							<li class="nav-item">
							  <button type="button" class="nav-link nav-link-appr" role="tab"
							    data-bs-toggle="tab" data-bs-target="#navs-top-approveHistory" data-tab="approveHistory"
							    aria-controls="navs-top-profile" aria-selected="false">승인</button>
							</li>
							<li class="nav-item">
							  <button type="button" class="nav-link nav-link-appr" role="tab"
							    data-bs-toggle="tab" data-bs-target="#navs-top-rejectHistory" data-tab="rejectHistory"
						...
						<div class="d-flex justify-content-between align-items-center">
						    ...
						    <div class="input-group input-group-merge apprSearch">
						        <input type="search" id="detailsSearch" placeholder="제목검색" class="form-control" value="">
						        <button type="button" id="btnDetailsSearch" class="input-group-text"><i class="ti ti-search"></i></button>
						    </div>
						</div>
						<!-- // 건수/검색 영역 -->
