...
$(function(){
    ...
    getList(currentPage, keyword);
 	...
 	// 검색 버튼 클릭 시 리스트 가져오기
    $('#btnProgressSearch').on('click', function(){
        keyword = $('#progressSearch').val();
        getList(currentPage, keyword);
    });
    
    // 검색란에서 엔터 키를 누르면 리스트 가져오기
    $('#progressSearch').on('keyup', function(event){
        ...
        <div class="col flex-grow-0 approval-list">
			...
			<div class="d-flex justify-content-between align-items-center">
			    ...
			    <div class="input-group input-group-merge apprSearch">
			        <input type="search" id="progressSearch" placeholder="제목검색" class="form-control" value="">
			        <button type="button" id="btnProgressSearch" class="input-group-text"><i class="ti ti-search"></i></button>
			    </div>
			</div>
			<!-- // 건수/검색 영역 -->
