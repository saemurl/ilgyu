...
$(function(){
    ...
    getList(currentPage, keyword);
 	...
 	// 검색 버튼 클릭 시 리스트 가져오기
    $('#btnUpcomingSearch').on('click', function(){
        keyword = $('#upcomingSearch').val();
        getList(currentPage, keyword);
    });
    
    // 검색란에서 엔터 키를 누르면 리스트 가져오기
    $('#upcomingSearch').on('keyup', function(event){
...
function getList(currentPage, keyword){
	...
	$.ajax({
		...
		success:function(result){
			...
			if(result.total == 0){
				str += `<td colspan="6" class="center" style="height:100px">결재예정 문서가 없습니다.</td>`;
			}
        ...
        <div class="col flex-grow-0 approval-list">
	        ...
	        <div class="d-flex justify-content-between align-items-center">
			    ...
			    <div class="input-group input-group-merge apprSearch">
			        <input type="search" id="upcomingSearch" placeholder="제목검색" class="form-control" value="">
			        <button type="button" id="btnUpcomingSearch" class="input-group-text"><i class="ti ti-search"></i></button>
			    </div>
			</div>
			<!-- // 건수/검색 영역 -->
