...
(function () {
  ...
  function EarningReportsBarChart(arrayData, highlightData, labelArray) {
    const basicColor = config.colors_label.primary,
      highlightColor = config.colors.primary;
    var colorArr = [];
    let unit = '건'
    if(arrayData == attendanceAvgList) unit = 'h';

    for (let i = 0; i < arrayData.length; i++) {
      if (arrayData[i] === highlightData) {
  ...
  let ymax = 0;
  $.ajax({
  	url:"/attendance/getLeaveStaticByDepartAndMonth",
  	type:"get",
  	data:data,
    success:function(result){
      result.forEach(function(map, idx){
        yearlyLeaveList.push(map.COUNT);
        ylabelList.push(map.MONTH);
        if(map.COUNT > ymax) ymax = map.COUNT;
      })
  ...
  let attendanceAvgList = [];
  let alabelList = [];
  let amax = 0;
  $.ajax({
    url:"/attendance/getAttendanceAvgByDepartAndMonth",
    type:"get",
    data:data,
    success:function(result){
      result.forEach(function(map, idx){
        attendanceAvgList.push(map.AVGH);
        alabelList.push(map.MONTH);
        if(map.AVGH > amax) amax = map.AVGH;
      })
  ...
  let omax = 0;
  $.ajax({
    url:"/attendance/getOverTimeStaticByDepartAndMonth",
    type:"get",
    data:data,
    success:function(result){
      result.forEach(function(map, idx){
        overTimeList.push(map.COUNT);
        olabelList.push(map.MONTH);
        if(map.COUNT > omax) omax = map.COUNT;
      })
  ...
  $.ajax({
		url:"/attendance/getTodayStaticByStts",
		type:"get",
		data:data,
		success:function(result){
			console.log(result);
			result.forEach(function(map, idx){
				labels.push(map.STTS + "(" + map.COUNT + "건)");
				datas.push(map.COUNT);
				total += Number(map.COUNT);
				if (map.STTS === '근무') {
                	workCount = Number(map.COUNT);
            	}
			})
  const deliveryExceptionsChartE1 = document.querySelector('#todayAttendanceChart'),
            ...
            labels: {
              ...
              total: {
                ...
                formatter: function (w) {
                  let workRatio = total > 0 ? (workCount / total * 100).toFixed(1) : 0;
                  return workRatio + '%';
                }
              }
            }
