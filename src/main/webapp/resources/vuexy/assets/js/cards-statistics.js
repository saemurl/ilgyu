...
(function () {
  ...
  let labelList = [];
  let total = 0;
  fetch("/approval/getLastWeekStatic")
    .then((resp) => {
      resp.json().then((data) => {
        console.log(data);
        data.forEach(function(map, idx) {
          countList.push(map.COUNT);
          labelList.push(map.WEEKDAY);
          total += Number(map.COUNT);
        });
        
        let staticTotalEl = document.querySelector('#staticTotal');
        if (staticTotalEl) {
          staticTotalEl.innerHTML = `${total}<small>건</small>`;
        }
      });
    });
